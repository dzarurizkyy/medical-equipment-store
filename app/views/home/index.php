...
<div class="d-flex flex-column align-items-center gap-4 mb-3">
  <!-- Container -->
  <div class="container d-flex w-full">
    <!-- Product -->
    <div class="d-flex flex-row flex-wrap justify-content-start w-75 gap-4">
      <?php foreach($data["products"] as $product) : ?>
        ...
        <div class="card p-1" style="width: 30%; height: <?= (isset($data["product_id"]) && (int)$data["product_id"] === $product["id"]) ? '22rem' : '19rem' ?>">
          ...
          <?php if(isset($data["product_id"]) && (int)$data["product_id"] === $product["id"] ) : ?>
            <form action="<?= BASEURL ?>/home/order/<?= $product["id"] ?>" method="post" class="row g-2 px-3">
              <!-- Input -->
              <div class="col-9">
    ...
    <!-- Filter -->
    <div class="card px-2 d-flex gap-2" style="width: 340px; border: none;">
      <!-- Title -->
